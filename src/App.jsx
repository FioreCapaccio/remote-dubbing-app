...
const App = () => {
  ...
  // ADR Scripting
  const [adrMarkers, setAdrMarkers] = useState([]);
  ...
  const handleStartProcess = useCallback(() => {
    ...
    recordStartTime.current = videoRef.current ? videoRef.current.currentTime : currentTime;
    startRecording();
  ...
  }, [isRecording, stopRecording, startRecording, currentTime]);

  const handleRemoteCommand = useCallback((cmd) => {
    switch (cmd.type) {
      case 'PLAY': videoRef.current?.play(); setIsPlaying(true); break;
      case 'PAUSE': videoRef.current?.pause(); setIsPlaying(false); break;
      case 'REC_START': handleStartProcess(); break;
      case 'SEEK': 
        if (videoRef.current) {
  ...
  const { peerId, isConnected, sendCommand, remoteStream, startTalkback, stopTalkback } = usePeerSession(roomName, sessionRole, handleRemoteCommand);
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (isResizingHorizontal.current) setSidebarWidth(Math.max(200, Math.min(e.clientX, 500)));
    ...
    const handleMouseUp = () => {
      isResizingVertical.current = false; isResizingHorizontal.current = false; isScrubbing.current = false;
  ...
  }, [duration, handleTogglePlay, selectedClipId]);
  ...
  const handleExportMixdown = async () => {
    ...
    } catch (e) { alert("Export error"); } finally { setIsExporting(false); }
  };
    ...
    <ErrorBoundary>
      <div className="app-container" onMouseUp={() => setDraggingClip(null)}>
        <DawSidebar 
          ...
          isConnected={isConnected} peerId={peerId} devices={devices}
          ...
          adrMarkers={adrMarkers} setAdrMarkers={setAdrMarkers}
          addAdrMarker={() => setAdrMarkers([...adrMarkers, { id: Date.now(), time: currentTime, text: '' }].sort((a,b) => a.time - b.time))}
        ...
        <main className="main-content">
          <DawTransport 
            ...
            currentTime={currentTime} videoURL={videoURL} videoFileName={videoFileName}
          ...
          <VideoPreview 
            ...
            activeMarker={adrMarkers.slice().reverse().find(m => m.time <= currentTime && currentTime - m.time < 3)}
          ...
          <DawTimeline 
            ...
            isScrubbing={isScrubbing} duration={duration} sidebarWidth={sidebarWidth}
            zoomLevel={zoomLevel} videoRef={videoRef} setCurrentTime={setCurrentTime}
            adrMarkers={adrMarkers} setAdrMarkers={setAdrMarkers} tracks={tracks} setTracks={setTracks}
